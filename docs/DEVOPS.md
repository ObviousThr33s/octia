...
It also publishes a **release card**, generated by `tools/release-card.ps1` and
used as the release body with the commit notes appended under it. A jar is not a
playtest: whoever downloads one still has to reconcile four versions that must
agree, and then they spawn somewhere ordinary and may never meet a watershed, a
derelict or the squid at all. The card carries the four versions, the install
steps, the fact that Octia is opt-in on the world-creation screen, a seed that
was actually walked and measured rather than guessed, and how to send a verdict
back.

Every number in it is **read from `gradle.properties`, never typed** — the same
argument the tag check above makes, applied to the notes instead of the jar. A
card is published once per tag and then outlives the memory of whoever cut it,
so a hand-copied Fabric API version is a support burden with a delay fuse on it.
The script refuses to write a card at all if a version is missing, because a card
with a blank where a version belongs reads as authoritative and is not. It is the
same script the owner runs locally, for the same reason the two gates are one
gate.
